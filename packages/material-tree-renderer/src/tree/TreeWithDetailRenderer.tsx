...
const mapStateToProps = (state: JsonFormsState, ownProps: OwnPropsOfTreeControl & WithImageProvider & WithLabelProviders): StatePropsOfTreeWithDetail => {
  ...
  const visible = has(ownProps, 'visible') ? ownProps.visible :  Runtime.isVisible(ownProps.uischema, state);
  const enabled = has(ownProps, 'enabled') ? ownProps.enabled :  Runtime.isEnabled(ownProps.uischema, state);
